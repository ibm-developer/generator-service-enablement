...
def getService(app):
    config = {}
    SERVER_URL = IBMCloudEnv.getString('auth_oauthServerUrl')
    ...
    config['public-key'] = publicKeyJson
    config['pem'] = pem
    config['context'] = 'APPID_AUTH_CONTEXT'
    config['clientId'] = IBMCloudEnv.getString('auth_clientId')
    config['authorizationEndpoint'] = AUTH_URL
    config['secret'] = IBMCloudEnv.getString('auth_secret')
